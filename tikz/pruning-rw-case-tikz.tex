...
\begin{tikzpicture}[vertex/.style = {circle, draw, minimum size = 30pt},
  ...
  \uncover<2->{
  \node[vertex, below = 2.0cm of from, fill = yellow!50] (prec) {$\mathit{prec}$};

  \draw[edge] (prec) to[sloped] node[above, purple]{{\it not} \textsf{RW}} (from);
  \draw[path] (to) to[bend left = 70, sloped] (prec);
